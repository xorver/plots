...
# plot 1
plot(SyncBS, SyncTH, ylim=c(100, 400), xlab="Block Size [MB]", col='blue', xaxt='n', log="x", ylab="Throughput [MB / s]")
axis(1, at=SyncBS, lab=SyncBS)
# with sync
arrows(SyncBS, SyncTH-SyncSD, SyncBS, SyncTH+SyncSD, col='blue', length=0.05, angle=90, code=3)
P <- predict(loess(SyncTH ~ SyncBS))
lines(P ~ SyncBS, col="blue", lwd=1, lty='dashed')
# without sync
points(NoSyncBS, NoSyncTH, pch=22, lty=2, col="red")
